...
> ⚠️ **Пересмотрено 2026-08-26.** Шаги ниже реализуют гейт `NETWORKS_WITHOUT_PAYER_DETECTION` как чёрный список шести сетей с fail-open для Tron, Taler и любой незнакомой сети — так и было в исходном плане, код ниже это отражает буквально. Владелец admin-API (Володя) это решение отменил в тот же день: «Пока давай ограничимся только Taler. Для остальных сетей пока не хватает фактуры» — Tron убирается из поддерживаемых (на нём определяется подписант транзакции, а не отправитель токенов, и это может быть кошелёк биржи), а гейт становится белым списком из одной сети `taler` с fail-closed для всего остального, включая незнакомые сети. См. актуальное поведение в `src/informer-bot/informer.refund.formatters.ts` (`NETWORKS_WITH_PAYER_DETECTION`) и в разделе «Гейт по сети» дизайн-документа. Шаги и код ниже оставлены как исторический TDD-протокол задачи и не отражают текущее поведение дословно.
