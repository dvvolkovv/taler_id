...
## ⚠️ Поправка к задачам 11-13 (внесена 2026-09-07, после реализации веба)

Задачи ниже писались против ошибочного предположения: будто ответ на собственный
`POST` приходит раньше, чем сервер разошлёт эхо в data-канал. Это неверно **по
устройству кода**, а не изредка: `sendRoomChatMessage` делает `await sendData(...)`
и только потом возвращает ответ — то есть пакет уходит в комнату всегда раньше.
Веб на этом обжёгся: сообщение рисовалось дважды либо навсегда оставалось на
чужой стороне.

Что из этого следует для мобильной части:

1. **Не помечать `msgId` обработанным «сразу после ответа»** — этого места в
   тексте задач 12 и 13 больше нет, оно ошибочно. Клиент присылает свой
   `clientMsgId` в теле запроса, сервер возвращает его же в пакете эха
   отдельным полем, и автор узнаёт своё сообщение без гонки: рисует пузырь
   сразу, а эхо с известным `clientMsgId` пропускает.
2. **Не принимать решение об отрисовке по дедуп-множеству.** В вебе гейт
   смотрел в набор `msgId`, а тот при переполнении очищается целиком — дубль
   возвращался примерно каждые 200 сообщений. Источник правды о том, «нарисовано
   ли уже», — сама лента, а не набор идентификаторов. Значит `RoomChatMessage`
   обязан нести `msgId`.
3. **История и живые пакеты разъезжаются по порядку.** Лента дорисовывается в
   конец, поэтому пакет, пришедший за время загрузки истории, окажется выше
   неё и уже не исправится. В вебе окно узкое, на мобильном интернете —
   особенно через RU-edge — широкое. Историю класть в начало ленты (у
   контроллера для этого есть `setHistory`) либо придерживать живые пакеты до
   её применения.
4. **Автор истории теперь известен.** `GET` отдаёт у каждого сообщения `own`;
   сырой идентификатор отправителя наружу не уходит. Разбор истории обязан это
   поле читать, иначе после переподключения все свои сообщения станут чужими —
   а на телефоне переподключение штатно.
5. **История может обогнать эхо и принести твоё же сообщение.** Человек пишет,
   пока ещё летит первоначальный запрос ленты; ответ приходит раньше эха и уже
   содержит это сообщение. Признак `own` тут не спасает: он говорит «твоё», но
   не говорит «это ровно тот пузырь, который уже нарисован». Поэтому лента
   отдаёт и `clientMsgId` — по нему разбор истории обязан пропускать сообщения,
   которые клиент уже нарисовал сам. В вебе это окно — один круг по сети, на
   телефоне через RU-edge оно заметно шире.
6. **Ответ может потеряться, а сообщение — дойти.** С `clientMsgId` это больше
   не беда: пузырь уже нарисован, эхо опознано. Не возвращать текст в поле
   ввода по факту сетевой ошибки, если эхо этого сообщения уже пришло. И не
   удалять запись об ожидании при отказе — помечать: эхо может прийти позже
   ошибки, и тогда своё сообщение нарисуется чужим.
