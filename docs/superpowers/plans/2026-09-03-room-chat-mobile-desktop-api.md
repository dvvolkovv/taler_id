...
> **Итог после ревью (коммит `1de72d3`).** Код выше — стартовая точка; по результатам ревью качества метод получил ещё три вещи, и именно эта форма поехала дальше:
> - `name` нормализуется в сервисе, а не только в контроллере: `(typeof name === 'string' ? name.trim() : '').slice(0, 64) || 'Taler ID'`. После снятия гейта в вебе (Task 3, шаг 0) `name` становится единственным основанием для атрибуции серверного сообщения, и рисуется он как `textContent`;
> - `text` проверяется на тип, а не только на `null`: `typeof text === 'string' ? text.trim() : ''` — иначе `{"text": 42}` доезжает до сервиса (глобальный `ValidationPipe` не срабатывает на не-классовых метатипах) и даёт 500 вместо 400;
> - отказ `sendData` логируется через `console.error` и пробрасывается дальше; поле `ok` из возврата убрано — литеральный `true`, который никогда не бывает `false`, изображал дискриминант, которым не являлся. Тип стал `Promise<{ ts: number }>`.
>
> Тестов стало 13 вместо 7. Комментарий про CIS-развилку переписан: маршрутизация через `sfuFor` сохранена ради симметрии с `deleteRoom`/`joinRoom`, но сама развилка отключена 2026-07-25 (`void region` в `createRoom`), и производителей имён `call-ru-*` в репозитории больше нет.
