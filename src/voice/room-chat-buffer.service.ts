...
export class RoomChatBufferService {
   ...
   * молчания.
   *
   * Это не безусловная гарантия — у неё есть свой горизонт. Счётчик не
   * бессмертен: у него свой TTL (см. `seqKey` в `append()`), и он тихо
   * истекает, если следующая встреча в этой комнате не случится вовсе или
   * случится позже, чем через сутки. Если счётчик успеет истечь до того,
   * как начнётся следующая встреча, `append()` заново стартует нумерацию с
   * единицы (`INCR` несуществующего ключа Redis) — и старый застрявший
   * курсор снова столкнётся ровно с той же тихой тишиной, от которой
   * спасает этот метод: `read()` не отличает «счётчик пережил очистку» от
   * «счётчик истёк и начался заново», ему просто негде взять старое
   * значение. Это принято как есть, а не исправлено: горизонт совпадает с
   * TTL самой ленты (`ttlSeconds`) — если ассистент не получал ни одного
   * ответа сутки, есть чем насторожиться и без `truncated`, а комната,
   * которой не пользовались сутки, и так вне зоны, которую эта лента
   * обязана покрывать.
   *
