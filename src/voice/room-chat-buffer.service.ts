...
export interface RoomChatEntry {
  ...
  actor?: string;
  /**
   * Тот же клиентский id, что клиент прислал в `POST .../chat` и что уходит
   * эхом в пакет data-канала (см. докстринг `sendRoomChatMessage`) — только
   * когда он прошёл `isValidClientMsgId` и есть `actor` (то же условие, что
   * решает и namespaced `msgId`, и эхо в пакете: источник истины один).
   *
   * Хранится и, в отличие от `actor`, отдаётся клиенту через GET как есть
   * (см. `RoomChatEntryOut`) — это безобидная случайная строка, придуманная
   * самим клиентом, и никаких сведений об участниках она не несёт. Нужна,
   * чтобы клиент опознавал в истории уже нарисованный им пузырь: `own`
   * говорит «моё», но не говорит «ровно тот пузырь, что уже на экране» —
   * баг воспроизводился так: пока летит первый `GET`, пользователь отправляет
   * сообщение; сервер успевает дописать его в ленту до того, как придёт
   * echo из data-канала, и `GET`-ответ приходит с этим сообщением раньше
   * echo — без `clientMsgId` в ответе клиенту нечем было сопоставить строку
   * истории с уже отрисованным оптимистичным пузырём, и он рисовал второй.
   *
   * Опционален: не было или не прошло проверку — поля нет вовсе, ни в
   * записи, ни (значит) в GET-ответе — не `null`, не пустая строка. Клиент
   * отличает «не было id» от «был» по наличию ключа.
   */
  clientMsgId?: string;
}
 ...
 * запрашивающего, см. `RoomChatBufferService.read`. `clientMsgId` (если
 * есть у сообщения) передаётся насквозь как есть — см. её докстринг в
 * `RoomChatEntry`.
 */
...
export class RoomChatBufferService {
  ...
  async read(
    ...
    // Отдельно: курсор строго больше latest — тоже дыра, а не «новых
    // сообщений нет». Это ловит две вещи одной строкой: курсор из будущего
    // (`?since=99999999`, клиент утверждает, что видел то, чего не было) и
    // тот случай, когда seq-счётчик истёк собственным TTL и следующая
    // встреча в этой комнате начала нумерацию заново с единицы (см.
    // докстринг `clearFeed` про горизонт этой гарантии) — тогда `latest`
    // мал, а старый `since` велик, и без этой строки ответ выглядел бы как
    // тишина. Флаг и так объявлен осторожным («может сработать вхолостую»,
    // см. RoomChatPage.truncated) — сравнение мимо реального будущего не
    // противоречит этой семантике.
    const truncated =
      since > latest ||
   ...
   * молчания.
   *
   * Счётчик тоже не бессмертен: у него свой TTL (см. `seqKey` в
   * `append()`), и он тихо истекает, если следующая встреча в этой комнате
   * не случится вовсе или случится позже, чем через сутки. Если счётчик
   * успеет истечь до того, как начнётся следующая встреча, `append()`
   * заново стартует нумерацию с единицы (`INCR` несуществующего ключа
   * Redis) — старый застрявший курсор снова оказался бы *старше* нового
   * `latest`. Раньше это означало ту же тихую тишину, от которой спасает
   * этот метод, только добравшуюся другим путём. Больше не означает:
   * `read()` явно ловит и это условие — `since > latest` в его расчёте
   * `truncated` (I4) — так что курсор дальше того, что реально есть в
   * комнате, теперь везде даёт честный `truncated: true`, а не молчание: у
   * этой гарантии больше нет горизонта, за которым она переставала бы
   * действовать.
   *
