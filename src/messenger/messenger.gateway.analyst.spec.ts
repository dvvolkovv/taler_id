...
describe('MessengerGateway._dispatchToAnalyst', () => {
  ...
  function replayWorker(
    callbacks: SubmitTaskInput,
    script: Array<['tool', string, string] | ['chunk', string]>,
  ) {
  ...
  it('emits typing event for each tool with localized label', async () => {
    ...
    await (gateway as any)._dispatchToAnalyst('user-1', 'conv-1', 'hi', []);
    const typingEvents = emitted.filter(
      (e) => e.event === 'typing' && e.data.isTyping,
    );
  ...
  it('emits analyst_seam after saving final message', async () => {
    ...
    expect(seam!.data.steps).toEqual(
      expect.arrayContaining([{ kind: 'file', count: 1 }]),
    );
  });
  ...
  it('emits typing isTyping=false to clear indicator after done', async () => {
    ...
    await (gateway as any)._dispatchToAnalyst('user-1', 'conv-1', 'hi', []);
    const clears = emitted.filter(
      (e) => e.event === 'typing' && e.data.isTyping === false,
    );
    expect(clears.length).toBeGreaterThanOrEqual(1);
  });

  it('on error, emits error typing and saves error message', async () => {
    (mockAnalyst.submitTask as jest.Mock).mockRejectedValue(
  ...
  it('uses English labels when Profile.language=en', async () => {
    ...
    const searchTyping = emitted.find(
      (e) => e.event === 'typing' && e.data.typingText?.includes('🔍'),
    );
  ...
  it('times out after 3 minutes and emits error typing', async () => {
    jest.useFakeTimers();
    (mockAnalyst.submitTask as jest.Mock).mockImplementation(
      () => new Promise(() => {}),
    );
