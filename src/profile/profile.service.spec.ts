...
describe('ProfileService', () => {
  ...
  describe('updateProfile', () => {
    ...
    it('throws NotFoundException when profile does not exist', async () => {
      ...
      await expect(
        service.updateProfile('missing-user', { firstName: 'Jane' }),
    ...
    it('converts dateOfBirth string to Date object', async () => {
      mockPrisma.profile.findUnique.mockResolvedValue({ ...PROFILE });
      let captured: any;
      mockPrisma.profile.update.mockImplementation(async (args: any) => {
        captured = args;
        return { ...PROFILE };
      });
  ...
  describe('linkWallet', () => {
    ...
    it('accepts lowercase hex address', async () => {
      ...
      await expect(
        service.linkWallet('user-1', { walletAddress: addr }),
  ...
  describe('uploadDocument', () => {
    ...
    it('throws BadRequestException when file exceeds 10MB', async () => {
      ...
      await expect(
        service.uploadDocument(
          'user-1',
          bigFile as Express.Multer.File,
          'PASSPORT',
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('accepts file exactly at 10MB limit', async () => {
      ...
      await expect(
        service.uploadDocument(
          'user-1',
          exactFile as Express.Multer.File,
          'PASSPORT',
        ),
      ).resolves.toBeDefined();
    });
    ...
    it('throws BadRequestException for invalid MIME type', async () => {
      ...
      await expect(
        service.uploadDocument(
          'user-1',
          invalidMime as Express.Multer.File,
          'PASSPORT',
        ),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('accepts image/png MIME type', async () => {
      ...
      await expect(
        service.uploadDocument(
          'user-1',
          pngFile as Express.Multer.File,
          'NATIONAL_ID',
        ),
      ).resolves.toBeDefined();
    });
    ...
    it('accepts application/pdf MIME type', async () => {
      ...
      await expect(
        service.uploadDocument(
          'user-1',
          pdfFile as Express.Multer.File,
          'DIPLOMA',
        ),
      ).resolves.toBeDefined();
    });
    ...
    it('saves s3Key with documents/ prefix', async () => {
      ...
      mockPrisma.document.create.mockImplementation(async (args: any) => {
        captured = args;
        return { id: 'doc-1', type: 'PASSPORT', uploadedAt: new Date() };
      });
    ...
    it('accepts all valid document types', async () => {
      ...
      for (const docType of validTypes) {
        ...
        await expect(
          service.uploadDocument('user-1', validFile, docType),
        ).resolves.toBeDefined();
      }
    });
  });
  ...
  describe('getDocuments', () => {
    ...
    it('throws NotFoundException when profile does not exist', async () => {
      ...
      await expect(service.getDocuments('missing-user')).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('getDocumentDownloadUrl', () => {
    it('returns presigned URL for existing document', async () => {
      ...
      mockPrisma.document.findFirst.mockResolvedValue({
        id: 'doc-1',
        s3Key: 'documents/user-1/file.jpg',
      });
    ...
    it('throws NotFoundException when profile does not exist', async () => {
      ...
      await expect(
        service.getDocumentDownloadUrl('missing-user', 'doc-1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws NotFoundException when document does not exist', async () => {
      ...
      await expect(
        service.getDocumentDownloadUrl('user-1', 'no-such-doc'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('calls S3 getPresignedUrl with 300s expiry', async () => {
      ...
      mockPrisma.document.findFirst.mockResolvedValue({
        id: 'doc-1',
        s3Key: 'documents/user-1/file.pdf',
      });
      mockS3.getPresignedUrl.mockResolvedValue('https://s3.example.com/signed');
  ...
  describe('deleteDocument', () => {
    ...
    it('throws NotFoundException when profile does not exist', async () => {
      ...
      await expect(
        service.deleteDocument('missing-user', 'doc-1'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('throws NotFoundException when document does not exist', async () => {
      ...
      await expect(
        service.deleteDocument('user-1', 'no-such-doc'),
      ).rejects.toThrow(NotFoundException);
    ...
    it('calls S3 deleteFile before DB delete', async () => {
      ...
      mockS3.deleteFile.mockImplementation(async () => {
        callOrder.push('s3');
      });
      mockPrisma.document.delete.mockImplementation(async () => {
        callOrder.push('db');
        return { id: 'doc-1' };
      });
