...
ОТВЕТ НА ПИСЬМО: адресат ответа — ВСЕГДА fromAddress (адрес отправителя) из check_mail/read_mail, НИКОГДА не адрес собственного ящика пользователя и не поле to. Если адреса отправителя в данных нет — сначала вызови read_mail по uid этого письма. Тему начинай с «Re: ».
