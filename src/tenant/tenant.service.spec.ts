...
describe('TenantService', () => {
  ...
  describe('createTenant', () => {
    it('creates tenant and returns it with OWNER member', async () => {
      const tenantWithMember = {
        ...TENANT,
        members: [makeMember(OWNER_ID, TenantRole.OWNER)],
      };
      mockPrisma.tenant.create.mockResolvedValue(tenantWithMember);

      const result = await service.createTenant(OWNER_ID, {
        name: 'Acme Corp',
      });
    ...
    it('returns tenants with role for the user', async () => {
      mockPrisma.tenantMember.findMany.mockResolvedValue([
        { role: TenantRole.OWNER, tenant: { ...TENANT } },
        {
          ...
          tenant: { ...TENANT, id: 'tenant-2', name: 'Beta LLC' },
        },
      ]);
  ...
  describe('getTenant', () => {
    ...
    it('throws ForbiddenException when user is not a member', async () => {
      ...
      await expect(service.getTenant(TENANT_ID, 'outsider')).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
  ...
  describe('updateTenant', () => {
    it('allows OWNER to update tenant', async () => {
      ...
      const result = await service.updateTenant(TENANT_ID, OWNER_ID, {
        name: 'Updated Corp',
      });
    ...
    it('allows ADMIN to update tenant', async () => {
      mockPrisma.tenantMember.findFirst.mockResolvedValue(
        makeMember(ADMIN_ID, TenantRole.ADMIN),
      );
      mockPrisma.tenant.update.mockResolvedValue({
        ...TENANT,
        name: 'Updated Corp',
      ...
      await expect(
        service.updateTenant(TENANT_ID, ADMIN_ID, { name: 'Updated Corp' }),
      ).resolves.toBeDefined();
    });

    it('throws ForbiddenException when user is VIEWER (not OWNER/ADMIN)', async () => {
      mockPrisma.tenantMember.findFirst.mockResolvedValue(
        makeMember(VIEWER_ID, TenantRole.VIEWER),
      );

      await expect(
        service.updateTenant(TENANT_ID, VIEWER_ID, { name: 'Hack' }),
    ...
    it('throws ForbiddenException when user is not a member at all', async () => {
      ...
      await expect(
        service.updateTenant(TENANT_ID, 'outsider', { name: 'Hack' }),
      ).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('startKyb', () => {
    ...
    it('throws ForbiddenException when user is not OWNER', async () => {
      ...
      await expect(service.startKyb(TENANT_ID, ADMIN_ID)).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('throws ForbiddenException when user is not a member', async () => {
      ...
      await expect(service.startKyb(TENANT_ID, 'outsider')).rejects.toThrow(
        ForbiddenException,
      );
    });
  });
  ...
  describe('handleKybWebhook', () => {
    it('updates kybStatus to VERIFIED on GREEN review', async () => {
      ...
      expect(mockPrisma.tenant.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ kybStatus: KycStatus.VERIFIED }),
        }),
      );
    });
  ...
  describe('inviteMember', () => {
    ...
    it('throws ForbiddenException when requester is VIEWER (not OWNER/ADMIN)', async () => {
      ...
      await expect(
        service.inviteMember(TENANT_ID, VIEWER_ID, {
          email: 'x@example.com',
          role: 'VIEWER',
        }),
      ).rejects.toThrow(ForbiddenException);
  ...
  describe('acceptInvite', () => {
    ...
    it('creates member and deletes invite on valid accept', async () => {
      ...
      mockPrisma.tenantMember.create.mockResolvedValue(
        makeMember('invitee-id', TenantRole.VIEWER),
      );
      ...
      expect(mockPrisma.pendingInvite.delete).toHaveBeenCalledWith({
    ...
    it('throws NotFoundException when invite token not found', async () => {
      ...
      await expect(service.acceptInvite('bad-token', 'user-1')).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('throws BadRequestException when invite is expired', async () => {
      ...
      await expect(
        service.acceptInvite(INVITE_TOKEN, 'invitee-id'),
    ...
    it('throws ForbiddenException when user email does not match invite email', async () => {
      ...
      await expect(
        service.acceptInvite(INVITE_TOKEN, 'wrong-user'),
      ).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('changeRole', () => {
    it('OWNER can change VIEWER role to ADMIN', async () => {
      ...
      mockPrisma.tenantMember.update.mockResolvedValue(
        makeMember(VIEWER_ID, TenantRole.ADMIN),
      );

      const result = await service.changeRole(TENANT_ID, OWNER_ID, VIEWER_ID, {
        role: 'ADMIN',
      });
    ...
    it('throws NotFoundException when target member not found', async () => {
      ...
      await expect(
        service.changeRole(TENANT_ID, OWNER_ID, 'ghost-user', {
          role: 'ADMIN',
        }),
    ...
    it('OWNER can change ADMIN to VIEWER', async () => {
      ...
      mockPrisma.tenantMember.update.mockResolvedValue(
        makeMember(ADMIN_ID, TenantRole.VIEWER),
      );

      const result = await service.changeRole(TENANT_ID, OWNER_ID, ADMIN_ID, {
        role: 'VIEWER',
      });
  ...
  describe('removeMember', () => {
    ...
    it('throws ForbiddenException when trying to remove OWNER', async () => {
      ...
      await expect(
        service.removeMember(TENANT_ID, OWNER_ID, TARGET_OWNER_ID),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('throws NotFoundException when target member not found', async () => {
      ...
      await expect(
        service.removeMember(TENANT_ID, OWNER_ID, 'ghost-user'),
      ).rejects.toThrow(NotFoundException);
    });

    it('throws ForbiddenException when requester is VIEWER', async () => {
      mockPrisma.tenantMember.findFirst.mockResolvedValueOnce(
        makeMember(VIEWER_ID, TenantRole.VIEWER),
      );

      await expect(
        service.removeMember(TENANT_ID, VIEWER_ID, 'someone'),
      ).rejects.toThrow(ForbiddenException);
    });
  ...
  describe('getKybStatus', () => {
    ...
    it('throws ForbiddenException when user is not a member', async () => {
      ...
      await expect(service.getKybStatus(TENANT_ID, 'outsider')).rejects.toThrow(
        ForbiddenException,
      );
    });
    ...
    it('throws NotFoundException when tenant does not exist', async () => {
      ...
      await expect(service.getKybStatus(TENANT_ID, OWNER_ID)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
