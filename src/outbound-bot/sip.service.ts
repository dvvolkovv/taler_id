...
export class SipService {
  ...
  private readonly sipClient = new SipClient(
    LK_HOST,
    LK_API_KEY,
    LK_API_SECRET,
  );
  ...
  async dialOutbound(
    ...
    this.logger.log(
      `[SIP] dialOutbound room=${roomName} phone=${phoneNumber} trunk=${SIP_TRUNK_ID}`,
    );
    try {
      ...
      return {
        participantId: result?.participantIdentity || `sip-${phoneNumber}`,
      };
    } catch (e) {
