...
jest.mock('otplib', () => ({
  ...
  generateURI: jest.fn(
    () =>
      'otpauth://totp/Taler%20ID:test@example.com?secret=MOCKSECRET&issuer=Taler%20ID',
  ),
...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('creates user and returns tokens', async () => {
      ...
      const result = await service.register(
        { email: 'test@example.com', password: 'Password1x' },
        '127.0.0.1',
        'Jest',
      );
    ...
    it('hashes password before storing', async () => {
      ...
      mockPrisma.user.create.mockImplementation(async ({ data }) => {
        captured = data;
        return { id: 'u1', email: data.email, phone: null };
      });
      mockPrisma.session.create.mockResolvedValue({ id: 'session-1' });
      mockPrisma.kycRecord.findUnique.mockResolvedValue({
        status: 'UNVERIFIED',
      ...
      await service.register(
        { email: 'hash@example.com', password: 'PlainTextX' },
        '127.0.0.1',
        'Jest',
      );
    ...
    it('stores refresh token in Redis with TTL', async () => {
      ...
      await service.register(
        { email: 'r@example.com', password: 'Password1x' },
        '127.0.0.1',
        'Jest',
      );
  ...
  describe('login', () => {
    ...
    it('throws UnauthorizedException for non-existent user', async () => {
      ...
      await expect(
        service.login(
          { email: 'nobody@example.com', password: 'Password1x' },
          '127.0.0.1',
          'Jest',
        ),
      ).rejects.toThrow(UnauthorizedException);
    ...
    it('throws ForbiddenException when account is locked', async () => {
      ...
      await expect(
        service.login(
          { email: 'locked@example.com', password: 'Password1x' },
          '127.0.0.1',
          'Jest',
        ),
    ...
    it('returns 2fa challenge when TOTP is enabled', async () => {
      ...
      mockPrisma.user.findFirst.mockResolvedValue({
        ...
        phone: null,
        passwordHash: hash,
        totpSecret: { secret: 'TOTP_SECRET', verified: true },
      });
      mockRedis.get.mockResolvedValue(null);
      mockRedis.setEx.mockResolvedValue('OK');
      mockRedis.del.mockResolvedValue(1);
      ...
      const result = await service.login(
        { email: 'totp@example.com', password: 'Password1x' },
        '127.0.0.1',
        'Jest',
      );
    ...
    it('increments failed attempts on wrong password', async () => {
      ...
      await expect(
        service.login(
          { email: 'fail@example.com', password: 'wrong' },
          '127.0.0.1',
          'Jest',
        ),
      ).rejects.toThrow(UnauthorizedException);
  ...
  describe('refreshTokens', () => {
    ...
    it('throws UnauthorizedException for revoked session', async () => {
      ...
      await expect(
        service.refreshTokens('some-token', '127.0.0.1', 'Jest'),
      ).rejects.toThrow(UnauthorizedException);
    });
  ...
  describe('verify2FA', () => {
    it('throws UnauthorizedException for invalid challenge token', async () => {
      mockRedis.get.mockResolvedValue(null);
      await expect(
        service.verify2fa('bad-token', '123456', '127.0.0.1', 'UA'),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('throws UnauthorizedException when user has no TOTP', async () => {
      ...
      await expect(
        service.verify2fa('tok', '123456', '127.0.0.1', 'UA'),
      ).rejects.toThrow(UnauthorizedException);
    });
    ...
    it('throws UnauthorizedException for invalid TOTP code', async () => {
      ...
      mockPrisma.auditLog.create.mockResolvedValue({});
      await expect(
        service.verify2fa('tok', '000000', '127.0.0.1', 'UA'),
      ).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('verifyTotp', () => {
    it('throws BadRequestException when TOTP not set up', async () => {
      ...
      await expect(
        service.verifyTotp('u1', '123456', '127.0.0.1', 'UA'),
    ...
    it('throws UnauthorizedException for invalid code', async () => {
      const otplib = require('otplib');
      otplib.verify.mockReturnValueOnce(false);
      mockPrisma.totpSecret.findUnique.mockResolvedValue({
        secret: 'JBSWY3DPEHPK3PXP',
        verified: false,
      });
      mockPrisma.auditLog.create.mockResolvedValue({});
      await expect(
        service.verifyTotp('u1', '000000', '127.0.0.1', 'UA'),
      ).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('disableTotp', () => {
    it('throws UnauthorizedException when password is wrong', async () => {
      ...
      mockPrisma.auditLog.create.mockResolvedValue({});
      await expect(
        service.disableTotp('u1', 'wrong', '127.0.0.1', 'UA'),
      ).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('revokeSession', () => {
    it('throws ForbiddenException for session belonging to another user', async () => {
      ...
      await expect(
        service.revokeSession('u1', 's1', '127.0.0.1', 'UA'),
      ).rejects.toThrow(ForbiddenException);
    });
